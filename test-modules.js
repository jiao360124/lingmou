...
try {
  ...
  setTimeout(() => {
    ...
    results.coreModules.push({
      ...
      passed: status.status !== 'CRITICAL' || status.heapUsagePercentage < 90
    });
  }, 3000);
} catch (e) {
  results.coreModules.push({
...
try {
  ...
  evaluator.evaluate({}).then(result => {
    results.strategies.push({
...
try {
  ...
  const calculator = new BenefitCalculator();
  const result = calculator.calculate({});
  results.strategies.push({
...
try {
  ...
  const result = analyzer.analyze({ cost: 100, benefit: 200 });
  results.strategies.push({
    module: 'roi-analyzer',
