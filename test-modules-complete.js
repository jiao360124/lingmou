...
try {
  ...
  generator.generateScenarios({
    strategies: ['FAST_RESPONSE', 'BALANCED', 'AGGRESSIVE', 'CONSERVATIVE', 'ADAPTIVE'],
    count: 3
  }).then(scenarios => {
...
try {
  const RiskController = require('./core/strategy/risk-controller');
  const controller = new RiskController({ maxRisk: 0.7 });
  const result = controller.control({
    riskLevel: 'HIGH',
    action: 'mitigate'
  });
  results.strategies.push({
    module: 'risk-controller',
    passed: result !== null && result.action !== undefined,
...
try {
  const RiskAdjustedScorer = require('./core/strategy/risk-adjusted-scorer');
  const scorer = new RiskAdjustedScorer();
  const result = scorer.calculate({
    baseScore: 80,
    riskLevel: 'MEDIUM'
  });
  results.strategies.push({
    module: 'risk-adjusted-scorer',
    passed: result.score !== undefined,
    score: result ? result.score : null
  });
} catch (e) {
  results.strategies.push({
...
try {
  const AdversarySimulator = require('./core/strategy/adversary-simulator');
  const simulator = new AdversarySimulator();
  const result = simulator.simulate({
    scenario: 'defense'
  });
  results.strategies.push({
    module: 'adversary-simulator',
    passed: result !== null && result.strength !== undefined,
    strength: result ? result.strength : null
  });
} catch (e) {
  results.strategies.push({
    module: 'adversary-simulator',
...
try {
  const MultiPerspectiveEvaluator = require('./core/strategy/multi-perspective-evaluator');
  const evaluator = new MultiPerspectiveEvaluator();
  const result = evaluator.evaluate({
    scenarios: [{ score: 80 }, { score: 90 }]
  });
  results.strategies.push({
    module: 'multi-perspective-evaluator',
    passed: result !== null && result.overallScore !== undefined,
