...
async function scheduleManager() {
  try {
    ...
    while (true) {
      ...
      const choice = await prompt('请输入选项 (0-7): ');
      ...
      switch (choice) {
        case '1':
          ...
          break;
        case '2':
          await enableTask(config);
          break;
        case '3':
          await disableTask(config);
          break;
        case '4':
...
async function enableTask(config) {
  displayTasks(config);
  const taskId = await prompt('请输入要启用的任务ID: ');
...
async function disableTask(config) {
  displayTasks(config);
  const taskId = await prompt('请输入要禁用的任务ID: ');
  const task = config.tasks.find(t => t.id === taskId);
...
async function executeTask(config) {
  displayTasks(config);
  const taskId = await prompt('请输入要执行的任务ID: ');
  const task = config.tasks.find(t => t.id === taskId);
  ...
  if (!task.enabled) {
    console.log('❌ 任务当前已禁用');
    const action = await prompt('是否强制执行？(y/n): ');
