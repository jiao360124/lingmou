/**
 * OpenClaw V3.2 - Core Strategy Engine 测试
 * 测试核心策略引擎各模块功能
 *
...
async function runTests() {
  console.log('═════════════════════════════════════════════════════════════');
  console.log('   OpenClaw V3.2 Core Strategy Engine 测试');
  console.log('═════════════════════════════════════════════════════════════\n');
  ...
  try {
    ...
    try {
      ...
      if (result && result.selected && result.selected.type === StrategyType.CONSERVATIVE) {
        console.log('✅ 保守型策略生成成功\n');
        console.log(`  策略类型: ${result.selected.label}`);
        console.log(`  延迟: ${result.selected.delay}ms`);
        console.log(`  压缩等级: ${result.selected.compressionLevel}`);
        console.log(`  模型偏置: ${result.selected.modelBias}`);
        console.log(`  估算成本: ${result.selected.estimatedCost}`);
        console.log(`  预期成功率: ${(result.selected.expectedSuccessRate * 100).toFixed(2)}%`);
        console.log(`  风险等级: ${result.selected.risk.level}`);
        console.log(`  综合得分: ${result.selected.combinedScore.toFixed(2)}`);
        console.log();
        passedTests++;
      } else {
    ...
    let balancedMetrics, balancedContext;
    ...
    try {
      ...
      result = await engine.generateAndSelectStrategy(
        balancedMetrics,
        balancedContext,
        constraints
      );

      if (result && result.selected && result.selected.type === StrategyType.BALANCED) {
    ...
    try {
      ...
      if (result && result.selected && result.selected.benefit && result.selected.benefit.totalScore >= 0 && result.selected.benefit.totalScore <= 100) {
    ...
    try {
      ...
      if (result.selected.risk && result.selected.risk.score >= 0 && result.selected.risk.score <= 100) {
        console.log('✅ 风险评估成功\n');
        console.log(`  风险分数: ${result.selected.risk.score.toFixed(2)}`);
        console.log(`  风险等级: ${result.selected.risk.level}`);
        console.log(`  风险详情:`);
        console.log(`    - 成功率风险: ${result.selected.risk.details.successRateRisk.toFixed(4)}`);
        console.log(`    - 成本比率: ${result.selected.risk.details.costRatio.toFixed(4)}`);
        console.log(`    - 延迟比率: ${result.selected.risk.details.delayRatio.toFixed(4)}`);
        console.log(`    - 压缩等级: ${result.selected.risk.details.compression}`);
        console.log();
        passedTests++;
      } else {
        console.log('❌ 风险评估失败\n');
    ...
    try {
      ...
      if (result.selected && result.selected.delay <= 100) {
        console.log('✅ 约束检查成功\n');
        console.log(`  延迟: ${result.selected.delay}ms (约束: ≤100ms)`);
        console.log();
        passedTests++;
      } else {
        console.log('❌ 约束检查失败\n');
        console.log(`  结果延迟: ${result.selected?.delay}ms, 约束: 100ms`);
        failedTests++;
      }
    } catch (error) {
    ...
    try {
      ...
      const learnings = engine.getAllLearnings();
